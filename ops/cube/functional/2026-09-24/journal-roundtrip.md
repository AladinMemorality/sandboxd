...
Hosted CI run `36032027907` on `bfc36e0` also passed Go tests, both architecture E2E jobs, console checks, and Linux/macOS installation checks. The only subsequent executable-source change in this evidence commit adds the explicit Docker resource limits used in the actual journal run.
