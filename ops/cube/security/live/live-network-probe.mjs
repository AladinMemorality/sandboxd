// Disposable attached-worker isolation workload. See ../README.md.
...
async function probe() {
  const targets = await Promise.all(config.targets.map(async target => {
    ...
    const rebound = answers.length ? await tcp(target.domain, target.port, { lookup: (_, __, cb) => cb(null, answers[0], 4) }) : { connected: false, error: 'missing_dns_answer' };
  ...
  const report = {
    ...
    unapprovedDomain: await tcp('example.com', 443),
