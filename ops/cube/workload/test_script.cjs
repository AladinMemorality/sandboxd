...
function fixture() {
  ...
  vm.runInNewContext(script, {
    ...
    process:{pid:42,memoryUsage(){return {rss:513*1024*1024};},cpuUsage(){return {user:clock,system:1};},resourceUsage(){return {minorPageFault:touched,majorPageFault:0};},exit(code){assert.equal(code,0);killed=true;}},
...
assert.equal(f.request('GET','/health').body.faults.minor,131072);assert.equal(f.request('GET','/health').body.faults.major,0);
