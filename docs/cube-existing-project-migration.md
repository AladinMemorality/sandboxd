...
inventory is not eligible for unattended migration: applications have owner data
outside `workspace/app` requiring a compatibility adapter. No production container was stopped or copied in this work.
...
migration journal exists. `inventory`, `status`, and `rollback-check` remain read-only and available
...
| `quiesced` | Source stopped; write private app and selected-task archives |
...
| `rollback_archived` | Current app/history archives/checksums durable; restore source |
| `rollback_restored` | Source copy verified; pause target and switch provider |
| `rolled_back` | Same identity restored to stopped Docker; both archives retained |
...
configuration currently blocks rollback before quiescence because the retained
Docker container would otherwise reuse stale/revoked environment values. Such a
project needs a Docker recreation adapter before rollback is eligible.
...
cube-migrate --database /DATA/state/sandboxd.db --workspaces /DATA/workspaces \
  ...
  --migrations /REVIEWED/migrations --sandbox SANDBOX_ID --preset react-vite migrate
...
Transport bounds are256MiB compressed,4GiB expanded,128MiB per file and200,000
...
silently copied or discarded. Larger streaming transfer and a reviewed home/tool
compatibility mechanism are necessary before fleet migration.

## Remaining owner-home compatibility gate
...
compatibility adapter, **not currently eligible**. The other apparent skeleton
...
rollback, corrupted archive rejection, changed-config rollback refusal,
...
transactions; hypervisor lifecycle calls are replaced. The opt-in
`TestLiveDockerCubeRollback` additionally uses real Docker/Cube and a disposable
...
projects were moved, and the production owner-home compatibility gate remains.
