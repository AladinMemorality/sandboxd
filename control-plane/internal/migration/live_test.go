...
// Task execution uses deterministic fake-opencode, not a real model execution claim.
func TestLiveDockerCubeRollback(t *testing.T) {
	if os.Getenv("CUBE_MIGRATION_LIVE") != "1" {
	...
	t.Cleanup(func() {
		if t.Failed() {
			db.Close()
			t.Logf("retained failed fixture: directory=%s container=%s", root, containerName)
			return
		}
		cleanup, cancel := context.WithTimeout(context.Background(), time.Minute)
		defer cancel()
		if m, e := db.GetRuntimeMigration(cleanup, id); e == nil && m.Binding.RuntimeID != "" {
			if e = client.Delete(cleanup, m.Binding.RuntimeID); e != nil {
	...
	engine := &Engine{Store: db, Backend: backend}
	started := time.Now()
	...
	newTask := ulid.Make().String()
	if err = db.CreateTask(ctx, &store.Task{TaskID: newTask, SandboxID: id, Agent: "opencode", Prompt: "run deterministic local migration fixture"}); err != nil {
		t.Fatal(err)
	}
	continueSession := false
	if err = guest.StartTask(ctx, runtime.StartTaskRequest{TaskID: newTask, Agent: "opencode", Prompt: "run deterministic local migration fixture", TimeoutS: 60, Continue: &continueSession}); err != nil {
		t.Fatal(err)
	}
	var newResult *runtime.TaskResult
	if err = wait(ctx, 90*time.Second, func() bool {
		result, e := guest.TaskResult(ctx, newTask)
		if e == nil && result.ID == newTask && result.Status != runtime.TaskRunning {
			newResult = result
			return true
		}
		return false
	...
	if newResult.Status != runtime.TaskSucceeded || newResult.CheckpointID == "" {
		t.Fatalf("real task fixture did not complete with checkpoint: %+v", newResult)
	}
	events, e := guest.TaskEvents(ctx, newTask, 0)
	if e != nil {
		t.Fatal(e)
	}
	done := false
	e = runtime.DecodeEvents(events, func(event runtime.Event) bool {
	...
	events.Close()
	if e != nil || !done {
		t.Fatal("real task completion events missing", e)
	}
	newJSON, _ := json.Marshal(newResult)
	if err = db.FinishTask(ctx, newTask, "succeeded", string(newJSON)); err != nil {
		t.Fatal(err)
	}
	started = time.Now()
	...
	for _, taskID := range []string{oldTask, newTask} {
	...
	t.Logf("real Docker→Cube→write+task completion→Docker passed: migrate=%dms rollback=%dms; deterministic agent fixture, no model call", migrationMS, rollbackMS)
